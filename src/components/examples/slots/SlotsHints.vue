  ...
  >
    <template #desc>
      <template v-if="!props.isEng">
        <p>
          Пример со слотами. В данном примере я делаю подсказки для маркированных областей, и вставляю логотип компании. Пример весьма условный, поэтому в качестве логотипа я взял логотип бетмена. Маркированные области нельзя создавать. Те маркированные области, которые есть, нельзя растягивать и переносить в пределах картинки. В моём компоненте есть 2 слота, "first" и "last". В слоте "first" я вывожу подсказки. В слот "last" я вставил логотип бетмена. Смотрите как это всё устроено, слот "first", он самый первый, за ним идут маркированные области (области которые мы будем создавать, или те области которые нам нужно отобразить), за ним идёт активная область и область в момент маркирования картинки, а дальше будет находиться слот "last". Он будет последним и будет перекрывать все другие слои. Мой компонент это svg-ка, поэтому вставлять в данные слоты нужно те элементы, которые допустимы в svg.
        </p>
      </template>
