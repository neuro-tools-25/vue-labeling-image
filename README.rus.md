...
### Настройки прерывистости линий для маркированных областей

Основным предназначением данных входных параметров является как-то выделить/отделить маркированные области от активной маркированной области, или области в момент маркирования документа. Иначе говоря привлечь внимание к нужной области. В SVG есть такой параметр как "stroke-dasharray", он отвечает за вид пунктирной обводки документов. В него можно передать числа разделённые пробелами, или запятыми. Если задать одно число, то это будет длина штриха и длина пробела между ними. Два числа будут означать, первое число длина штриха, а второе длина пробела между штрихами. Возможно задавать произвольное число цифр, к примеру 4 1 2 3, в этом случае длина штриха будет 4px, затем будет пробел в 1px, затем будет идти штрих в 2px и пробел после него в 3px, и так эти значения будут повторяться. К примеру, не на каждой теме можно понять какая маркированная область активная, а какая нет. В таком случае, обычные маркированные области можно сделать пунктиром, а для активной маркированной области пунктир можно не задавать, это должно привлечь внимание к активной маркированной области.

**draggingStrokeDasharray** - вид пунктирной обводки маркированной области в момент маркирования документа. Задавать нужно числа разделённые пробелами, или запятыми. Вы можете увидеть, на что влияет данный входной параметр только в момент маркирования документа.

**activeStrokeDasharray** - вид пунктирной обводки активной маркированной области. Она появится после того как вы создадите хотя бы одну маркированную область. Задавать нужно числа разделённые пробелами, или запятыми.

**strokeDasharray** - вид пунктирной обводки для общих маркированных областей. Чтобы увидеть, на что влияет данный входной параметр, необходимо создать хотя бы 2 маркированные области. Первая область будет активной, а ко второй и последующим будет применяться данный входной параметр.Задавать нужно числа разделённые пробелами, или запятыми.
...
Выше я уже описывал как нужно стилизовывать мой компонент, этот же раздел скорее всего нужен для каких-то экстренных случаев. Когда нужно, быстро поправить стили не трогая CSS, или выделить что-то в зависимости от каких-то условий. Для того, чтобы вам было попроще, я рекомендую вам открыть ["страницу стилизации"](https://neuro-tools-25.github.io/vue-labeling-image/stylization?lang=rus "Страница стилизации"), на данной странице можно будет поменять входные параметры, и посмотреть какой входной параметр за что отвечает.
...
### Входные параметры отвечающие за стилизацию блока маркирования картинок

В данном разделе идёт речь о входных параметрах отвечающих за стилизацию блока в котором будет находиться картинка для маркирования. Как стилизовать области для маркирования будет описано ниже.

**bg** - входной параметр отвечающий за background блока маркирования картинок

**border** - входной параметр отвечающий за border для блока маркирования картинок.

**boxShadow** - входной параметр отвечающий за box-shadow для блока маркирования картинок.
...
**labelingRectFill** - входной параметр отвечающий за цвет заливки в момент маркирования картинки.

**labelingRectStroke** - входной параметр отвечающий за цвет линии в момент маркирования картинки.

**labelingRectFillOpacity** - входной параметр отвечающий за прозрачность заливки в момент маркирования картинки.

**labelingRectStrokeOpacity** - входной параметр отвечающий за прозрачность линии в момент маркирования картинки.

### Входные параметры отвечающие за стилизацию активной маркированной области

Посмотреть что делают данные входные параметры можно после того, как процесс маркирования завершится. Последняя созданная область станет активной. Также данные стили будут применяться к области когда мы по ней кликнем. Параметр **isReadonly** должен находиться в значении false, иначе стили для активной области вы не увидите, да и разметить области вы не сможете. Так-же параметр **isMarkup** должен находиться в значении true, иначе картинку вы маркировать не сможете.

**activeRectFilll** - входной параметр отвечающий за заливку активной маркированной области.

**activeRectStroke** - входной параметр отвечающий за цвет линии активной маркированной области.

**activeRectFillOpacity** - входной параметр отвечающий за прозрачность активной маркированной области.

**activeRectStrokeOpacity** - входной параметр отвечающий за прозрачность линии активной маркированной области.
...
**rectFill** - входной параметр отвечающий за заливку маркированных областей.

**rectStroke** - входной параметр отвечающий за цвет линий маркированных областей.

**rectStrokeWidth** - входной параметр отвечающий за толщину линий маркированных областей.

**rectStrokeOpacity** - входной параметр отвечающий за прозрачность линий маркированных областей.

**rectFillOpacity** - входной параметр отвечающий за прозрачность заливки маркированных областей.

**rectRx** - входной параметр отвечающий за border-radius по оси X.

**rectRy** - входной параметр отвечающий за border-radius по оси Y.

В большинстве случаев **rectRy** и **rectRy** будут одинаковыми, если вам не нужны закругления, то установите значения в 0.

### Входные параметры отвечающие за стилизацию тени маркированных областей

В данном блоке представлены входные параметры отвечающие за цвет линии для тени, её толщину и прозрачность. При стилизации учитывайте, что цвет для тени должен быть конрастным с цветом линии маркированной области. В большинстве случаев тень стоит включать только тогда, когда вы не знаете какие точно документы будет загружать пользователь. К примеру, если загрузить документ с преобладанием красного, у нас в России это к примеру паспорт, то активная маркированная область будет не очень заметна. Тень нужна для того, чтобы выделить маркированную область на документе, сделать так, чтобы она не сливалась с ним.

**shadowRectStroke** - входной параметр отвечающий за цвет линий для тени.

**shadowRectStrokeWidth** - входной параметр отвечающий за толщину линии для тени.

**shadowRectStrokeOpacity** - входной параметр отвечающий за прозрачность линии тени.

### Входные параметры отвечающие за стилизацию сетки

Здесь представлен входной параметр отвечающий за цвет сетки. Чтобы увидеть за что отвечает данный входной параметр, входной параметр **"enableMarking"** должен находиться в значении true.

**gridColor** - входной параметр отвечающий за цвет линии сетки.
